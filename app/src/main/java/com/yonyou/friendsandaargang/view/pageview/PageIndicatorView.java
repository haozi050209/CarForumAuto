package com.yonyou.friendsandaargang.view.pageview;
...
public class PageIndicatorView extends LinearLayout {
    ...
    private int dotSize = 7; // 指示器的大小（dp）
    ...
    public void initIndicator(int count) {
        ...
        if (indicatorViews.size() > 0) {
            indicatorViews.get(0).setBackgroundResource(R.drawable.ddd);
        }
    }
    ...
    public void setSelectedPage(int selected) {
        for (int i = 0; i < indicatorViews.size(); i++) {
            if (i == selected) {
                indicatorViews.get(i).setBackgroundResource(R.drawable.ddd);
            } else {
                indicatorViews.get(i).setBackgroundResource(R.drawable.logo1);
            }
        }
    }
