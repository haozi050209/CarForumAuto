...
public class RoundedDrawable extends Drawable {
    ...
    public RoundedDrawable(Bitmap bitmap, int color) {
        mBitmap = bitmap;
    ...
    public static RoundedDrawable fromBitmap(Bitmap bitmap, int color) {
        if (bitmap != null) {
            return new RoundedDrawable(bitmap, color);
        } else {
            return null;
        }
    }

    public static Drawable fromDrawable(Drawable drawable, int color) {
        if (drawable != null) {
            if (drawable instanceof RoundedDrawable) {
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    ld.setDrawableByLayerId(ld.getId(i), fromDrawable(d, color));
                }
                return ld;
            }
            ...
            if (bm != null) {
                return new RoundedDrawable(bm, color);
            }
        }
        return drawable;
    }
